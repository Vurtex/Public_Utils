package com.puhua.crm.view;

import java.util.List;

import com.puhua.crm.R;
import com.puhua.crm.fragment.IndicatorFragment.TabInfo;
...
public class TitleIndicator extends LinearLayout implements View.OnClickListener, OnFocusChangeListener {
	...
	private static final float FOOTER_LINE_HEIGHT = 0;

	private static final int FOOTER_COLOR = 0xFFFFFFFF;

	private static final float FOOTER_TRIANGLE_HEIGHT = 0;
	...
	protected void add(int index, String label, int icon, boolean hasTips) {
		View tabIndicator;
		if (flag) {
			tabIndicator = mInflater.inflate(R.layout.title_flow_indicator_crm, this, false);
		}else{
			tabIndicator = mInflater.inflate(R.layout.title_flow_indicator_work, this, false);
		}
		...
		if(label!="")
		{
			...
			if (mTextColor != null) {
				...
				// tv.setBackgroundResource(R.drawable.selector_tab_background);
			}
		...
		ImageView tips = (ImageView) tabIndicator.findViewById(R.id.tab_title_tips);
//		if (icon > 0) {
...
//		}
	...
	public synchronized void setCurrentTab(int index) {
		...
		setTabTextSize(newTab, true,mSelectedTab);
	...
	private void setTabTextSize(View tab, boolean selected,int index) {
		...
		ImageView iv=(ImageView) tab.findViewById(R.id.tab_title_tips);
		if(getRes_hov()!=null&&getRes_nor()!=null)
		switch (index) {
		case 0:
			iv.setImageResource(selected ?getRes_hov()[0]:getRes_nor()[0]);
			break;
		case 1:
			iv.setImageResource(selected ?getRes_hov()[1]:getRes_nor()[1]);
			break;
		case 2:
			iv.setImageResource(selected ?getRes_hov()[2]:getRes_nor()[2]);
			break;
		case 3:
			iv.setImageResource(selected ?getRes_hov()[3]:getRes_nor()[3]);
			break;
		default:
			break;
		} 
	...
	public int[] getRes_nor() {
		return res_nor;
	}

	public void setRes_nor(int[] res_nor) {
		this.res_nor = res_nor;
	}

	public int[] getRes_hov() {
		return res_hov;
	}

	public void setRes_hov(int[] res_hov) {
		this.res_hov = res_hov;
	}
	public void setRes_hov(int[] res_hov,boolean flag) {
		this.res_hov = res_hov;
		this.flag=flag;
	}
