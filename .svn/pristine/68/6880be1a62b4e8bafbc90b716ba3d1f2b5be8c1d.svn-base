...
public final class VolleySSLSocketFactory {
	...
	public static MySSLSocketFactory getSSLSocketFactory (Context ctx) throws NoSuchAlgorithmException, KeyManagementException {
		return getSSLSocketFactory(ctx, null);
	}

	public static MySSLSocketFactory getSSLSocketFactory (Context ctx, CertificateEncryption encryption)
...
//		SSLContext context = SSLContext.getInstance("TLSv1.2");
		...
		try{
			...
			keyManagers = createKeyManagers( CertificateConfig.keyStoreFileName, CertificateConfig.keyStorePassword, CertificateConfig.alias, encryption);
			trustManagers = createTrustManagers( CertificateConfig.trustStoreFileName, CertificateConfig.trustStorePassword, encryption);
//			context.init(keyManagers, trustManagers, null);
		...
		MySSLSocketFactory socketFactory = new MySSLSocketFactory(keyManagers, trustManagers, null);
	
//		SSLSocket sslSocket = null;
//		try {
////			sslSocket = (SSLSocket) socketFactory.createSocket();
//			sslSocket = (SSLSocket) MySSLSocketFactory.getDefault().createSocket();
//		} catch (IOException e1) {
//			e1.printStackTrace();
//		};
//		
//		String[] s = sslSocket.getEnabledProtocols();
